class Quest14Controller < ApplicationController
  layout 'base'
  ...
  def teskalChart14
    ...
    if @answers.length > 1
      strXML = "<chart palette='2' caption='"+l(:quest14_label_0)+"' subCaption='"+@user.name+"' xAxisName='"+@fecha.to_s+"'showvalues='0'  decimalSeparator=',' formatNumberScale='0' legendAllowDrag='1' yAxisMinValue='0' yAxisMaxValue='10' showShadow='1'  useRoundEdges='1' showAlternateHGridColor='1' alternateHGridColor='f8f6f4'  numDivLines='4' bgcolor='ffffff' borderColor='ffffff'>"
      strXML = strXML +"<categories>"
      strXML = strXML + "<category label= '"+l(:quest5_label_1)+"'/>"
      strXML = strXML + "<category label= '"+l(:quest5_label_2)+"'/>"
      strXML = strXML + "<category label= '"+l(:quest5_label_3)+"'/>"
      strXML = strXML + "<category label= '"+l(:quest5_label_4)+"'/>"
      strXML = strXML +"</categories>"
    else
      strXML = "<chart caption='"+l(:quest14_label_0)+"' subCaption='"+@user.name+"' yAxisName='"+@fecha.to_s+"' palette='2' yAxisMaxValue='10' showvalues='0'  PYAxisName='Comarcas' formatNumberScale='0' legendAllowDrag='1' showShadow='1'  useRoundEdges='1' yAxisMaxValue='10'  showAlternateHGridColor='1' alternateHGridColor='f8f6f4' bgcolor='ffffff' borderColor='ffffff'>"
    end

    for answer in @answers
      rendimiento_general = (answer.answ1 + answer.answ2  + answer.answ3 + answer.answ4 + answer.answ5)
      rendimiento_emocional =  (answer.answ6 + answer.answ7 + answer.answ8 + answer.answ9 + answer.answ10)
      ...
      @icon=[]
      item=0
      if rendimiento_general < 14
        @advice[item]=l(:quest5_d1_a)
        @icon[item]="stop"
      else
        if rendimiento_general < 20
      ...
      item=1
      if rendimiento_emocional < 14
        @advice[item]=l(:quest5_d2_a)
        @icon[item]="stop"
      else
        if rendimiento_emocional < 20
      ...
      if @answers.length > 1
        ...
        strXML = strXML + "<set value='" + (rendimiento_general * scale).to_s + "'/>"
        strXML = strXML + "<set value='" + (rendimiento_emocional * scale).to_s + "'/>"
        strXML = strXML + "<set value='" + (rcog * scale).to_s + "'/>"
        strXML = strXML + "<set value='" +  (rcom * scale).to_s + "'/>"
        strXML = strXML + "</dataset>"
      else
        strXML = strXML + "<set label='" + l(:quest5_label_1) + "' value= '" + (rendimiento_general * scale).to_s + "'/>"
        strXML = strXML + "<set label='" + l(:quest5_label_2) + "' value= '" + (rendimiento_emocional * scale).to_s + "'/>"
        strXML = strXML + "<set label='" + l(:quest5_label_3) + "' value= '" + (rcog * scale).to_s + "'/>"
        strXML = strXML + "<set label='" + l(:quest5_label_4) + "' value= '" + (rcom * scale).to_s + "'/>"
      end
    end
    strXML = strXML + "</chart>"
    ...
    if @answers.length > 1
      #Create the chart - Pie 3D Chart with data from strXML
      @chart1= renderChart("/charts/MSCombi2D.swf"+l(:PBarLoadingText), "", strXML, "quest14", 780, 300, false, false)
    else
      @chart1= renderChart("/charts/Bar2D.swf"+l(:PBarLoadingText), "", strXML, "quest14", 550, 270, false, false)
    end
  end
end
